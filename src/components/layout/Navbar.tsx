...
export const Navbar: React.FC = () => {
  ...
  const { language, isRtl, currentUser } = useStore();
  ...
  return (
    <nav className="bg-hub-blue text-white shadow-md relative z-30">
      <div className="max-w-[1536px] mx-auto px-4 sm:px-6 flex items-center justify-between">
        {/* Left Links + All Categories Trigger */}
        <div className="flex items-center space-x-1 rtl:space-x-reverse overflow-x-auto scroll-smooth touch-pan-x [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden py-1 sm:py-0 flex-1 min-w-0 mr-2 rtl:mr-0 rtl:ml-2">
          {/* All Categories Dropdown Trigger */}
          <div className="relative shrink-0">
            <button
              onClick={() => setAllCategoriesOpen(!allCategoriesOpen)}
              className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-2.5 sm:py-3 hover:bg-hub-blue-dark transition-colors font-semibold text-[12px] sm:text-[14px] whitespace-nowrap rounded-lg sm:rounded-none"
            >
          ...
          {navLinks.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`px-3.5 py-3 text-[13px] sm:text-[14px] font-medium whitespace-nowrap transition-colors rounded-none hover:bg-hub-blue-dark ${
